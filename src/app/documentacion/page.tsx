...
export default function DocumentacionPage() {
  return (
    <div className="min-h-dvh bg-slate-50 text-slate-900">
      <header className="border-b bg-white">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          ...
          <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:items-center">
            <Link
              href="/"
              className="rounded-md border border-slate-300 bg-white px-3 py-2 text-center text-sm font-medium text-slate-700 hover:bg-slate-100"
            >
              Inicio
            </Link>
            <Link
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-3 py-5 sm:space-y-8 sm:px-6 sm:py-10">
        <section
          data-tour="doc-header"
          className="rounded-2xl border border-blue-100 bg-gradient-to-r from-blue-50 to-white p-5 sm:p-7"
        >
          <h1 className="text-xl font-bold sm:text-3xl">Guía rápida de módulos</h1>
          ...
          <div className="mt-5 grid grid-cols-2 gap-2 text-xs sm:flex sm:flex-wrap sm:gap-3 sm:text-sm">
            <a href="#panel" className="rounded-md border bg-slate-50 px-2 py-2 text-center">Panel</a>
            <a href="#directorio" className="rounded-md border bg-slate-50 px-2 py-2 text-center">Directorio</a>
            <a href="#iglesias" className="rounded-md border bg-slate-50 px-2 py-2 text-center">Iglesias</a>
            <a href="#ministerios" className="rounded-md border bg-slate-50 px-2 py-2 text-center">Ministerios</a>
            <a href="#asistencia" className="rounded-md border bg-slate-50 px-2 py-2 text-center">Asistencia</a>
            <a href="#ofrendas" className="rounded-md border bg-slate-50 px-2 py-2 text-center">Ofrendas</a>
            <a href="#inventario" className="rounded-md border bg-slate-50 px-2 py-2 text-center">Inventario</a>
            <a href="#configuracion" className="rounded-md border bg-slate-50 px-2 py-2 text-center">Configuración</a>
          </div>
        </section>

        <section data-tour="menu-library" className="rounded-2xl border bg-white p-4 sm:p-6">
          ...
          <div className="grid grid-cols-1 gap-3 min-[420px]:grid-cols-2 lg:grid-cols-4">
            {MENU_LIBRARY.map((item) => {
              ...
              return (
                <div
                  key={item.label}
                  className="group rounded-xl border border-slate-200 bg-slate-50/70 p-3 sm:p-4"
                >
        ...
        <section data-tour="doc-sections" className="grid gap-3 sm:gap-5">
          {DOC_SECTIONS.map((section) => (
            <article id={section.id} key={section.id} className="rounded-xl border bg-white p-4 sm:p-6">
                  ...
                  {section.id === 'panel' ? (
                    <div className="mt-4 overflow-hidden rounded-lg border border-slate-200 bg-white p-2">
                      <Image
                        ...
                        width={1600}
                        height={900}
                        className="h-auto max-h-[72vh] w-full rounded-md border object-contain"
                      />
                    </div>
                  ) : null}
                  {section.id === 'directorio' ? (
